...
function createMockHooks(overrides: Partial<Parameters<typeof runGatewayStartupSequence>[0]> = {}) {
  return {
    ...
    isSafeModeEnabled: vi.fn().mockReturnValue(false),
    enableSafeMode: vi.fn(),
    prepareForSafeModeRetry: vi.fn().mockResolvedValue(undefined),
...
describe('runGatewayStartupSequence', () => {
  ...
  it('switches to safe mode before transient retry for permission-like failures', async () => {
    let attempts = 0;
    const hooks = createMockHooks({
      findExistingGateway: vi.fn().mockResolvedValue(null),
      hasOwnedProcess: vi.fn().mockReturnValue(false),
      startProcess: vi.fn().mockImplementation(async () => {
        attempts++;
        if (attempts === 1) {
          throw new Error('spawn /app/resources/bin/uv EACCES');
        }
      }),
      isSafeModeEnabled: vi.fn()
    ...
    expect(hooks.enableSafeMode).toHaveBeenCalledTimes(1);
    expect(hooks.prepareForSafeModeRetry).toHaveBeenCalledTimes(1);
    expect(hooks.delay).toHaveBeenCalledWith(500);
    expect(hooks.startProcess).toHaveBeenCalledTimes(2);
    expect(hooks.onConnectedToManagedGateway).toHaveBeenCalledTimes(1);
  });

  it('cleans up before safe-mode retry when plugin failure is detected from stderr', async () => {
    let attempts = 0;
    const hooks = createMockHooks({
      findExistingGateway: vi.fn().mockResolvedValue(null),
      hasOwnedProcess: vi.fn().mockReturnValue(false),
      ...
      getStartupStderrLines: vi.fn()
        .mockReturnValueOnce(['failed to load plugin: wecom'])
        .mockReturnValueOnce([]),
      isSafeModeEnabled: vi.fn()
        .mockReturnValueOnce(false)
        .mockReturnValueOnce(true),
