...
function copyDirectoryContents(srcDir, destDir) {
  fs.mkdirSync(normWin(destDir), { recursive: true });
  for (const entry of fs.readdirSync(normWin(srcDir), { withFileTypes: true })) {
    const srcEntry = path.join(srcDir, entry.name);
    const destEntry = path.join(destDir, entry.name);
    const realSrc = entry.isSymbolicLink() ? fs.realpathSync(normWin(srcEntry)) : srcEntry;
    const stat = fs.statSync(normWin(realSrc));

    if (stat.isDirectory()) {
      copyDirectoryContents(realSrc, destEntry);
      continue;
    }

    fs.mkdirSync(normWin(path.dirname(destEntry)), { recursive: true });
    fs.copyFileSync(normWin(realSrc), normWin(destEntry));
    try {
      fs.chmodSync(normWin(destEntry), stat.mode);
    } catch {}
  }
}
...
// Note: pnpm's virtual store directory name (e.g. ".pnpm/openclaw@2026.4.2...")
// can become misleading when lockfiles/stores are in a mixed state. Always
// print the actual openclaw package.json version for clarity.
let openclawVersion = 'unknown';
try {
  const pkgJsonPath = path.join(openclawReal, 'package.json');
  const pkg = JSON.parse(fs.readFileSync(normWin(pkgJsonPath), 'utf8'));
  if (pkg && typeof pkg.version === 'string' && pkg.version.trim()) {
    openclawVersion = pkg.version.trim();
  }
} catch {}
echo`   openclaw version: ${openclawVersion}`;
...
copyDirectoryContents(openclawReal, OUTPUT);
...
for (const [realPath, pkgName] of collected) {
  ...
  try {
    ...
    copyDirectoryContents(realPath, dest);
    copiedCount++;
  } catch (err) {
      ...
      if (pkgEntry.name.startsWith('@')) {
        ...
        for (const scopeEntry of scopeEntries) {
          ...
          try {
            ...
            copyDirectoryContents(srcScoped, destScoped);
        ...
        try {
          copyDirectoryContents(srcPkg, destPkg);
