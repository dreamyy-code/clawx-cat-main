...
const UV_DOWNLOAD_BASE_CANDIDATES = [
  process.env.UV_DOWNLOAD_BASE_URL,
  `https://downloads.sourceforge.net/project/uv-project-manager.mirror/${UV_VERSION}`,
...
async function setupTarget(id) {
  ...
  try {
    // Download
    let lastError = null;
    for (const baseUrl of UV_DOWNLOAD_BASE_CANDIDATES) {
      const downloadUrl = `${String(baseUrl).replace(/\/+$/, '')}/${target.filename}`;
      try {
        ...
        lastError = error;
        echo(chalk.yellow`⚠️ Download failed, trying next source...`);
      }
    }
    if (lastError) {
      throw lastError;
