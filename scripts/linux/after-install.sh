...
# Ensure the main GUI binary keeps its executable bit after packaging.
if [ -f /opt/ClawX/clawx ]; then
    chmod 755 /opt/ClawX 2>/dev/null || true
    chmod 755 /opt/ClawX/clawx 2>/dev/null || true
    ln -sf /opt/ClawX/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Keep packaged helper binaries executable as well.
if [ -d /opt/ClawX/resources/bin ]; then
    find /opt/ClawX/resources/bin -type f -exec chmod 755 {} + 2>/dev/null || true
fi
