...
const PNG_SOURCE = path.join(ASSETS_DIR, 'favicon.png');
const FALLBACK_SVG_SOURCE = path.join(ICONS_DIR, 'icon.svg');

echo`🎨 Generating ClawX icons using project assets...`;

const hasPngSource = fs.existsSync(PNG_SOURCE);
const hasFallbackSvg = fs.existsSync(FALLBACK_SVG_SOURCE);

if (!hasPngSource && !hasFallbackSvg) {
  echo`❌ Icon source not found. Expected ${PNG_SOURCE} or ${FALLBACK_SVG_SOURCE}`;
  process.exit(1);
}
...
try {
  ...
  const rasterSource = hasPngSource ? PNG_SOURCE : FALLBACK_SVG_SOURCE;
  echo`  Processing icon source: ${path.relative(PROJECT_ROOT, rasterSource)}`;
  const masterPngBuffer = await sharp(rasterSource)
    .resize(1024, 1024)
    .png() // Ensure it's PNG
    .toBuffer();
  ...
  if (icoBuffer) {
    ...
    echo`  ✅ Created icon.ico from favicon.png`;
  } else {
