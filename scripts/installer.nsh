...
!macro preInit
  ; Prefer non-system drives by default to reduce permission and disk-space
  ; issues on C:, while still falling back to Program Files when needed.
  IfFileExists "D:\" 0 +3
    StrCpy $INSTDIR "D:\${PRODUCT_NAME}"
    Goto done
  IfFileExists "E:\" 0 +3
    StrCpy $INSTDIR "E:\${PRODUCT_NAME}"
    Goto done
  IfFileExists "F:\" 0 +3
    StrCpy $INSTDIR "F:\${PRODUCT_NAME}"
    Goto done
...
!macro customInstall
  ...
  ; Force the installed app to request elevation on launch via Windows
  ; compatibility flags. This avoids relying on executable resource editing,
  ; which currently fails on this machine due to legacy winCodeSign extraction.
  DetailPrint "Configuring the app to run as administrator by default..."
  ...
  ; Recreate shortcuts with the packaged favicon so the installed app shows the
  ; branded ClawX-Cat icon even when the executable resource icon is not edited.
  DetailPrint "Refreshing application shortcuts and icon..."
  SetShellVarContext all
  Delete "$DESKTOP\ClawX.lnk"
  Delete "$DESKTOP\${PRODUCT_NAME}.lnk"
  Delete "$SMPROGRAMS\ClawX.lnk"
  Delete "$SMPROGRAMS\${PRODUCT_NAME}.lnk"
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\${APP_EXECUTABLE_FILENAME}" "" "$INSTDIR\resources\resources\icons\icon.ico" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}.lnk" "$INSTDIR\${APP_EXECUTABLE_FILENAME}" "" "$INSTDIR\resources\resources\icons\icon.ico" 0
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "是否同时删除 ClawX-Cat 的应用数据？$\r$\n$\r$\n将删除以下目录：$\r$\n  • AppData\Local\clawx（本地应用数据）$\r$\n  • AppData\Roaming\clawx（漫游应用数据）$\r$\n$\r$\n你的 .openclaw 目录（配置与技能）会被保留。$\r$\n如果你希望下次重装时继续保留这些数据，请选择“否”。" \
