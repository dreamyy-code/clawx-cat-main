...
import {
  APP_BUILD_VARIANT,
  APP_OPENCLAW_TRACK,
  APP_OPENCLAW_VERSION,
  APP_UPDATE_BASE_URL,
  APP_UPDATE_TRACK,
} from '../utils/build-profile';
import { parseUpdateMetadataYaml, resolveUpdateMetadataFileName } from '../utils/update-metadata';
...
export class AppUpdater extends EventEmitter {
  ...
  constructor() {
    ...
    // alpha -> /proxy/alpha.yml, beta -> /main/beta-mac.yml, etc.
    const version = app.getVersion();
    const channel = detectChannel(version);
    const feedUrl = this.getFeedUrl();

    logger.info(
      `[Updater] Version: ${version}, variant: ${APP_BUILD_VARIANT}, openclawTrack: ${APP_OPENCLAW_TRACK}, openclawVersion: ${APP_OPENCLAW_VERSION}, channel: ${channel}, feedUrl: ${feedUrl}`,
    );

    // Set channel so electron-updater requests the correct yml filename.
    // e.g. channel "alpha" → requests alpha.yml / alpha-mac.yml from the
    // current variant directory, channel "latest" → requests latest.yml.
  ...
  private getChannel(): string {
    return detectChannel(app.getVersion());
  }

  private getFeedUrl(): string {
    const baseUrl = APP_UPDATE_BASE_URL;
    const track = APP_UPDATE_TRACK;
    if (process.platform === 'linux' && track) {
      return `${baseUrl}/${track}`;
    }
    return baseUrl;
  }

  private async checkForUpdatesInDev(): Promise<UpdateInfo | null> {
    this.updateStatus({ status: 'checking', error: undefined });

    const feedUrl = this.getFeedUrl();
    const channel = this.getChannel();
    const metadataFileName = resolveUpdateMetadataFileName(channel, process.platform, process.arch);
    const metadataUrl = `${feedUrl}/${metadataFileName}`;
    logger.info(`[Updater] Dev mode metadata check via ${metadataUrl}`);
    ...
    if (!response.ok) {
      throw new Error(`Dev update metadata request failed: HTTP ${response.status}`);
    }

    const rawMetadata = await response.text();
    const metadata = parseUpdateMetadataYaml(rawMetadata);
    const remoteVersion = String(metadata.version ?? '').trim();
    const currentVersion = app.getVersion();

    if (!remoteVersion) {
      throw new Error('Dev update metadata is missing a version field');
    }
    ...
    logger.info(
      `[Updater] Dev mode version compare: remote=${remoteVersion}, current=${currentVersion}, comparison=${comparison}`,
    );
  ...
  async checkForUpdates(): Promise<UpdateInfo | null> {
    try {
      if (!app.isPackaged) {
        return await this.checkForUpdatesInDev();
      }
  ...
  async downloadUpdate(): Promise<void> {
    try {
      if (!app.isPackaged) {
        throw new Error('Dev mode only supports update checking; downloading is disabled');
      }
  ...
  quitAndInstall(): void {
    ...
    if (!app.isPackaged) {
      throw new Error('Dev mode only supports update checking; installation is disabled');
    }
    setQuitting();
