...
import {
  importProviderConfigToOpenClaw,
  removeProviderFromOpenClaw,
  upsertManualProviderConfigToOpenClaw,
...
export async function handleProviderRoutes(
  ...
  if (url.pathname === '/api/provider-accounts/switch-upsert' && req.method === 'POST') {
    try {
      ...
      await upsertManualProviderConfigToOpenClaw(body);
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname === '/api/provider-accounts/switch-import' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ name: string; providerJson: Record<string, unknown> }>(req);
      await importProviderConfigToOpenClaw(body.name, body.providerJson);
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname === '/api/provider-accounts/switch-delete' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ accountId: string; providerKey: string }>(req);
      await providerService.deleteAccount(body.accountId);
      await removeProviderFromOpenClaw(body.providerKey);
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
