...
export async function handleAgentRoutes(
  ...
  if (url.pathname === '/api/agents/import' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ name: string; zipPath: string }>(req);
      if (!isImportAgentNameValid(body.name || '')) {
        throw new Error('Agent 名称仅支持英文、数字和英文符号');
      }
      const { snapshot, agentId } = await importAgentPackage(body.name, body.zipPath);
      syncAllProviderAuthToRuntime().catch((err) => {
        console.warn('[agents] Failed to sync provider auth after agent package import:', err);
      });
      syncAgentModelOverrideToRuntime(agentId).catch((err) => {
        console.warn('[agents] Failed to sync runtime model after agent package import:', err);
      });
      scheduleGatewayReload(ctx, 'import-agent-package');
      sendJson(res, 200, { success: true, importedAgentId: agentId, ...snapshot });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
    return true;
  ...
  if (url.pathname === '/api/agents/import/inspect' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ zipPath: string }>(req);
      const inspection = await inspectAgentPackage(body.zipPath || '');
  ...
  if (url.pathname === '/api/agents/communication' && req.method === 'PUT') {
    try {
      const body = await parseJsonBody<{ enabled?: boolean; allowedAgents?: string[] }>(req);
      const snapshot = await updateCommunicationConfig({
        enabled: body.enabled === true,
        allowedAgents: Array.isArray(body.allowedAgents) ? body.allowedAgents : [],
      });
      scheduleGatewayReload(ctx, 'update-communication');
      sendJson(res, 200, { success: true, ...snapshot });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname === '/api/agents/instructions/sync-all' && req.method === 'POST') {
    try {
      const snapshot = await syncAllAgentInstructions();
      sendJson(res, 200, { success: true, ...snapshot });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname.startsWith('/api/agents/') && req.method === 'POST') {
    ...
    if (parts.length === 2 && parts[1] === 'import-package') {
      try {
        ...
        const snapshot = await importAgentPackageIntoAgent(agentId, body.zipPath, {
          sourceAgentDirName: body.sourceAgentDirName,
          sourceWorkspaceDirName: body.sourceWorkspaceDirName,
        });
        syncAllProviderAuthToRuntime().catch((err) => {
          console.warn('[agents] Failed to sync provider auth after agent package import:', err);
        });
        ...
        scheduleGatewayReload(ctx, 'import-agent-package-into-existing');
        sendJson(res, 200, { success: true, importedAgentId: agentId, ...snapshot });
      } catch (error) {
        sendJson(res, 500, { success: false, error: String(error) });
      }
    ...
    if (parts.length === 2 && parts[1] === 'instructions') {
      try {
        const agentId = decodeURIComponent(parts[0]);
        const snapshot = await syncAgentInstructions(agentId);
        sendJson(res, 200, { success: true, ...snapshot });
      } catch (error) {
        sendJson(res, 500, { success: false, error: String(error) });
      }
  ...
  if (url.pathname.startsWith('/api/agents/') && req.method === 'PUT') {
    ...
    if (parts.length === 2 && parts[1] === 'communication') {
      try {
        const body = await parseJsonBody<{ spawnTargets?: string[] }>(req);
        const agentId = decodeURIComponent(parts[0]);
        const snapshot = await updateAgentCommunication(agentId, {
          spawnTargets: Array.isArray(body.spawnTargets) ? body.spawnTargets : [],
        });
        scheduleGatewayReload(ctx, 'update-agent-communication');
        sendJson(res, 200, { success: true, ...snapshot });
      } catch (error) {
        sendJson(res, 500, { success: false, error: String(error) });
      }
