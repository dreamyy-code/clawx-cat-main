...
export function copyPluginFromNodeModules(npmPkgPath: string, targetDir: string, npmName: string): void {
  const rawPkgPath = fsPath(npmPkgPath);
  const realPath = (() => {
    try {
      return realpathSync(rawPkgPath);
    } catch {
      return rawPkgPath;
    }
  })();
  ...
  if (!existsSync(realPath)) {
    throw new Error(`Plugin package path does not exist: ${npmPkgPath}`);
  }
  ...
  while (queue.length > 0) {
    ...
    for (const { name, fullPath } of listPackagesInDir(nodeModulesDir)) {
      ...
      let depRealPath = fsPath(fullPath);
      try {
        depRealPath = realpathSync(fsPath(fullPath));
      } catch {
        if (!existsSync(depRealPath)) continue;
      }
