...
const CONFIG_FILE_BACKUP = join(OPENCLAW_DIR, 'openclaw.json.bak');
...
export async function readOpenClawConfig(): Promise<OpenClawConfig> {
    ...
    try {
        ...
        try {
            if (await fileExists(CONFIG_FILE_BACKUP)) {
                const backupContent = await readFile(CONFIG_FILE_BACKUP, 'utf-8');
                const backupParsed = JSON.parse(backupContent) as OpenClawConfig;
                logger.warn('Recovered OpenClaw config from backup copy');
                return backupParsed;
            }
        } catch (backupError) {
            logger.error('Failed to read OpenClaw backup config', backupError);
        }
        return {};
    }
}
...
export async function writeOpenClawConfig(config: OpenClawConfig): Promise<void> {
    ...
    try {
        ...
        await writeFile(CONFIG_FILE_BACKUP, serialized, 'utf-8');
