...
export class WhatsAppLoginManager extends EventEmitter {
    ...
    private async buildCurrentQrPayload(): Promise<{ qr: string; raw: string } | null> {
        if (!this.qr) {
            return null;
        }
        const base64 = await renderQrPngBase64(this.qr);
    ...
    async waitForQr(timeoutMs: number = 20000): Promise<{ qr: string; raw: string }> {
        const current = await this.buildCurrentQrPayload();
        if (current) {
            return current;
        }
        return await new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                cleanup();
                reject(new Error('Timed out while waiting for WhatsApp QR code'));
            }, timeoutMs);

            const handleQr = (payload: { qr: string; raw: string }) => {
                cleanup();
                resolve(payload);
            };
            ...
            const handleError = (error: unknown) => {
                cleanup();
                reject(error instanceof Error ? error : new Error(String(error || 'WhatsApp login failed')));
            };

            const cleanup = () => {
                clearTimeout(timer);
                this.off('qr', handleQr);
    ...
    async startAndWaitForQr(accountId: string = 'default', timeoutMs: number = 20000): Promise<{ qr: string; raw: string }> {
        await this.start(accountId);
        return await this.waitForQr(timeoutMs);
    }
