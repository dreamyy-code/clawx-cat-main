...
import { ITERATIVECAT_DEFAULT_BASE_URL } from './build-profile';
...
function normalizeOpenClawProviderKey(
  rawKey: string,
  entry?: Record<string, unknown>,
): string {
  const key = normalizeConfigText(rawKey);
  const lowered = key.toLowerCase();
  const baseUrl = normalizeConfigText(entry?.baseUrl ?? entry?.baseURL).toLowerCase();
  const api = normalizeConfigText(entry?.api).toLowerCase();

  if (
    lowered === 'minimax'
    || lowered === 'minimax-portal-cn'
    || lowered === 'custom-minimax'
    || (api === 'anthropic-messages' && (baseUrl.includes('minimaxi.com/anthropic') || baseUrl.includes('minimax.io/anthropic')))
  ) {
...
function mergeProviderEntries(
  ...
  const leftModels = Array.isArray(left?.models) ? left.models : [];
  const rightModels = Array.isArray(right.models) ? right.models : [];
  if (leftModels.length > 0 || rightModels.length > 0) {
    const seen = new Set<string>();
    merged.models = [...leftModels, ...rightModels].filter((value) => {
...
function getNormalizedOpenClawProviders(
  ...
  for (const [rawKey, entry] of Object.entries(providers)) {
    const normalizedKey = normalizeOpenClawProviderKey(rawKey, entry);
    normalized[normalizedKey] = mergeProviderEntries(normalized[normalizedKey], entry);
  }
  return normalized;
}

function getNormalizedOpenClawProvidersSourceKeys(
  config: Record<string, unknown>,
  ...
  for (const [rawKey, entry] of Object.entries(providers)) {
    const normalizedKey = normalizeOpenClawProviderKey(rawKey, entry);
    if (!sourceKeys[normalizedKey]) {
      sourceKeys[normalizedKey] = rawKey;
    }
  }
  return sourceKeys;
}
...
export async function listOpenClawAuthProfileProviders(): Promise<string[]> {
  return Array.from(await getProvidersFromAuthProfileStores());
}

export async function getOpenClawModelSelection(): Promise<{
  primary?: string;
  fallbacks: string[];
}> {
  ...
  const defaults = agents?.defaults && typeof agents.defaults === 'object'
    ...
    : undefined;
  const modelConfig = defaults?.model && typeof defaults.model === 'object'
    ? (defaults.model as Record<string, unknown>)
    : undefined;

  return {
    primary: typeof modelConfig?.primary === 'string' ? modelConfig.primary : undefined,
    fallbacks: Array.isArray(modelConfig?.fallbacks)
      ? (modelConfig?.fallbacks as unknown[]).filter((value): value is string => typeof value === 'string')
      : [],
  };
}

export async function readOpenClawJsonForUi(): Promise<Record<string, unknown>> {
  return readOpenClawJson();
}

function ensureModelsProviders(config: Record<string, unknown>): Record<string, Record<string, unknown>> {
  const models = (
...
function ensureAuthProfiles(config: Record<string, unknown>): Record<string, Record<string, unknown>> {
  const auth = (
...
function ensureAuthProfileForProvider(config: Record<string, unknown>, providerName: string): void {
  const normalizedProvider = normalizeConfigText(providerName);
  if (!normalizedProvider) return;
  const profiles = ensureAuthProfiles(config);
  const hasExactProvider = Object.values(profiles).some((profile) => {
    if (!profile || typeof profile !== 'object') return false;
    return normalizeConfigText((profile as Record<string, unknown>).provider) === normalizedProvider;
  });
  if (hasExactProvider) return;
  profiles[`${normalizedProvider}:default`] = {
    provider: normalizedProvider,
    mode: 'api_key',
  };
}

function ensureAgentsModelConfig(config: Record<string, unknown>): Record<string, unknown> {
  const agents = (
...
function normalizeProviderModels(
  ...
  for (const entry of models) {
    ...
    if (typeof entry === 'string') {
      id = normalizeConfigText(entry);
      name = id;
    } else if (entry && typeof entry === 'object') {
      const record = entry as Record<string, unknown>;
      id = normalizeConfigText(record.id) || normalizeConfigText(record.name);
      name = normalizeConfigText(record.name) || id;
    }
...
export async function setOpenClawPrimaryModel(modelRef: string): Promise<void> {
  return withConfigLock(async () => {
    const config = await readOpenClawJson();
    const model = ensureAgentsModelConfig(config);
    model.primary = modelRef;
    if (Array.isArray(model.fallbacks)) {
      model.fallbacks = model.fallbacks.filter((value) => value !== modelRef);
    }
    await writeOpenClawJson(config);
  });
}

export async function addOpenClawFallbackModel(modelRef: string): Promise<void> {
  return withConfigLock(async () => {
    const config = await readOpenClawJson();
    const model = ensureAgentsModelConfig(config);
    const current = Array.isArray(model.fallbacks) ? [...model.fallbacks] : [];
    model.fallbacks = uniqueStringList([...current, modelRef]).filter((value) => value !== model.primary);
    await writeOpenClawJson(config);
  });
}

export async function removeOpenClawFallbackModel(modelRef: string): Promise<void> {
  return withConfigLock(async () => {
    const config = await readOpenClawJson();
    const model = ensureAgentsModelConfig(config);
    const current = Array.isArray(model.fallbacks) ? [...model.fallbacks] : [];
    model.fallbacks = current.filter((value) => value !== modelRef);
    await writeOpenClawJson(config);
  });
}

export async function addOpenClawProviderModel(providerKey: string, modelId: string): Promise<void> {
  return withConfigLock(async () => {
    const config = await readOpenClawJson();
    const providers = ensureModelsProviders(config);
    const provider = (
      providers[providerKey] && typeof providers[providerKey] === 'object'
        ? { ...(providers[providerKey] as Record<string, unknown>) }
        : {}
    ) as Record<string, unknown>;
    const currentModels = normalizeProviderModels(provider.models);
    if (!currentModels.some((item) => item.id === modelId)) {
      currentModels.push({ id: modelId, name: modelId });
    }
...
export async function removeOpenClawProviderModel(providerKey: string, modelId: string): Promise<void> {
  return withConfigLock(async () => {
    const config = await readOpenClawJson();
    const providers = ensureModelsProviders(config);
    ...
    const currentModels = normalizeProviderModels(provider.models);
    provider.models = currentModels.filter((value) => value.id !== modelId);
    providers[providerKey] = provider;

    const model = ensureAgentsModelConfig(config);
    if (model.primary === `${providerKey}/${modelId}`) {
      delete model.primary;
    }
    if (Array.isArray(model.fallbacks)) {
      model.fallbacks = model.fallbacks.filter((value) => value !== `${providerKey}/${modelId}`);
    }
    await writeOpenClawJson(config);
  });
}
  ...
  try {
    await withConfigLock(async () => {
      ...
      for (const rawKey of Object.keys(providers)) {
        const normalizedKey = normalizeOpenClawProviderKey(
          rawKey,
          providers[rawKey] && typeof providers[rawKey] === 'object'
            ? (providers[rawKey] as Record<string, unknown>)
            : undefined,
        );
        if (rawKey === provider || normalizedKey === provider) {
          delete providers[rawKey];
          modified = true;
          console.log(`Removed OpenClaw provider config: ${rawKey}`);
        }
      }
...
function normalizeProviderModelsForOpenClaw(models: unknown): Array<{ id: string; name: string }> {
  if (!Array.isArray(models)) return [];
  const seen = new Set<string>();
  ...
  for (const item of models) {
    ...
    if (!id || seen.has(id)) continue;
    seen.add(id);
    normalized.push({ id, name: name || id });
  }
  return normalized;
}

export async function upsertManualProviderConfigToOpenClaw(options: {
  name: string;
  baseUrl: string;
  api: string;
  apiKey?: string | null;
  ...
  const providerName = normalizeConfigText(options.name);
  ...
  await withConfigLock(async () => {
    ...
    const nextModels = normalizeProviderModelsForOpenClaw(existing.models);
    const modelId = normalizeConfigText(options.modelId);
    ...
    providers[providerName] = {
      ...existing,
      baseUrl: normalizeConfigText(options.baseUrl),
      api: normalizeConfigText(options.api),
      models: nextModels,
      ...(options.apiKey ? { apiKey: normalizeConfigText(options.apiKey) } : {}),
    };
    ensureAuthProfileForProvider(config, providerName);
  ...
  if (options.apiKey) {
    await saveProviderKeyToOpenClaw(providerName, normalizeConfigText(options.apiKey));
  }
}

export async function importProviderConfigToOpenClaw(name: string, providerJson: Record<string, unknown>): Promise<void> {
  const providerName = normalizeConfigText(name);
  if (!providerName) throw new Error('Missing provider name');

  const normalizedProvider = {
    ...providerJson,
    baseUrl: normalizeConfigText(providerJson.baseUrl ?? providerJson.baseURL),
    api: normalizeConfigText(providerJson.api),
    models: normalizeProviderModelsForOpenClaw(providerJson.models),
    ...(providerJson.apiKey ? { apiKey: normalizeConfigText(providerJson.apiKey) } : {}),
  };
  ...
  await withConfigLock(async () => {
    ...
    providers[providerName] = normalizedProvider;
    ensureAuthProfileForProvider(config, providerName);
    models.providers = providers;
    config.models = models;
    await writeOpenClawJson(config);
  ...
  const apiKey = normalizeConfigText(providerJson.apiKey);
  if (apiKey) {
    await saveProviderKeyToOpenClaw(providerName, apiKey);
  }
}

function normalizeIterativeCatOrigin(baseUrl: string): string {
  const trimmed = normalizeConfigText(baseUrl);
  if (!trimmed) return ITERATIVECAT_DEFAULT_BASE_URL;
  const noTrail = trimmed.replace(/\/+$/, '');
  if (noTrail.endsWith('/v1beta')) return noTrail.slice(0, -7);
  if (noTrail.endsWith('/v1')) return noTrail.slice(0, -3);
  return noTrail;
}

function uniqModels(models: string[]): Array<{ id: string; name: string }> {
  return Array.from(new Set(models.map((item) => normalizeConfigText(item)).filter(Boolean))).map((id) => ({ id, name: id }));
}

function resolveIterativeCatProviderKeyForModel(modelId: string): string {
  const normalized = modelId.toLowerCase();
  if (normalized.includes('claude')) return 'api-iterativecat-claude';
  if (normalized.includes('gemini')) return 'api-iterativecat-google';
  return 'api-iterativecat-gpt';
}

export async function configureIterativeCatProvidersToOpenClaw(options: {
  baseUrl: string;
  apiKey: string;
  primaryModelId: string;
}): Promise<void> {
  const origin = normalizeIterativeCatOrigin(options.baseUrl);
  const apiKey = normalizeConfigText(options.apiKey);
  const primaryModelId = normalizeConfigText(options.primaryModelId) || 'gpt-5.4';

  const primaryProviderKey = resolveIterativeCatProviderKeyForModel(primaryModelId);

  const gptModels = uniqModels([
    'gpt-5.4',
    primaryProviderKey === 'api-iterativecat-gpt' ? primaryModelId : '',
  ]);
  const claudeModels = uniqModels([
    'claude-sonnet-4-6',
    primaryProviderKey === 'api-iterativecat-claude' ? primaryModelId : '',
  ]);
  const googleModels = uniqModels([
    'gemini-3-flash-preview',
    primaryProviderKey === 'api-iterativecat-google' ? primaryModelId : '',
  ]);

  await withConfigLock(async () => {
    const config = await readOpenClawJson();
    const providers = ensureModelsProviders(config);
    providers['api-iterativecat-gpt'] = {
      baseUrl: `${origin}/v1`,
      api: 'openai-completions',
      apiKey,
      models: gptModels,
    };
    providers['api-iterativecat-claude'] = {
      baseUrl: origin,
      api: 'anthropic-messages',
      apiKey,
      models: claudeModels,
    };
    providers['api-iterativecat-google'] = {
      baseUrl: `${origin}/v1beta`,
      api: 'google-generative-ai',
      apiKey,
      models: googleModels,
    };
    const modelConfig = ensureAgentsModelConfig(config);
    modelConfig.primary = `${primaryProviderKey}/${primaryModelId}`;
    const fallbackCandidates = [
      gptModels[0] ? `api-iterativecat-gpt/${gptModels[0].id}` : '',
      claudeModels[0] ? `api-iterativecat-claude/${claudeModels[0].id}` : '',
      googleModels[0] ? `api-iterativecat-google/${googleModels[0].id}` : '',
    ].filter((item) => item && item !== modelConfig.primary);
    modelConfig.fallbacks = uniqueStringList(fallbackCandidates);
    await writeOpenClawJson(config);
  });

  await saveProviderKeyToOpenClaw('api-iterativecat-gpt', apiKey);
  await saveProviderKeyToOpenClaw('api-iterativecat-claude', apiKey);
  await saveProviderKeyToOpenClaw('api-iterativecat-google', apiKey);
}
...
export async function getActiveOpenClawProviders(): Promise<Set<string>> {
  ...
  try {
    ...
    // 1. models.providers
    const providers = getNormalizedOpenClawProviders(config);
...
export async function getOpenClawProvidersConfig(): Promise<{
  ...
  try {
    ...
    const providers = getNormalizedOpenClawProviders(config);
    const sourceKeys = getNormalizedOpenClawProvidersSourceKeys(config);
    ...
    return { providers, defaultModel, sourceKeys };
  } catch {
    return { providers: {}, defaultModel: undefined, sourceKeys: {} };
  }
}
