...
import { ensurePackagedLinuxHelperExecutables } from './linux-permissions';
...
async function runPythonInstall(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    child.on('error', (err) => {
      const noExecHint = process.platform === 'linux' && /EACCES|permission denied/i.test(err.message)
        ? '\n  hint: Linux reported EACCES when launching uv. Ensure the app is extracted on an executable filesystem (not mounted with noexec) and that resources/bin files have +x permissions.'
        : '';
      reject(new Error(
