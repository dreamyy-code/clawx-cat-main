...
function normalizeOpenClawTextValue(value: unknown): string {
  if (typeof value !== 'string') return '';
  return value.trim().replace(/^`+|`+$/g, '').trim();
}

function extractOpenClawModelId(value: unknown): string | undefined {
  if (typeof value === 'string') {
    const normalized = normalizeOpenClawTextValue(value);
    if (!normalized) return undefined;
    return normalized.includes('/') ? normalized.split('/').slice(1).join('/') : normalized;
  }
  if (value && typeof value === 'object') {
    const record = value as Record<string, unknown>;
    const id = normalizeOpenClawTextValue(record.id);
    if (id) return id;
    const name = normalizeOpenClawTextValue(record.name);
    if (name) return name;
  }
  return undefined;
}
...
export class ProviderService {
  ...
  async listAccounts(): Promise<ProviderAccount[]> {
    ...
    const { providers: openClawProviders, defaultModel, sourceKeys } = await getOpenClawProvidersConfig();
    const activeProviderKeys = new Set(Object.keys(openClawProviders));
    ...
    // Remove any store account whose runtime key no longer exists in openclaw.json.
    for (const account of allStoreAccounts) {
      const runtimeKey = getOpenClawProviderKeyForType(account.vendorId, account.id);
      if (!activeProviderKeys.has(runtimeKey)) {
        logger.info(
          `[provider-sync] Removing stale account "${account.id}" because runtime key "${runtimeKey}" is absent from openclaw.json`,
        );
        await deleteProviderAccount(account.id);
      }
    }
    ...
    // For each provider in openclaw.json, produce exactly ONE account.
    for (const key of activeProviderKeys) {
      ...
      if (storeGroup.length > 0) {
        ...
        let kept = storeGroup.find((account) => account.id === key) ?? storeGroup[0];
        ...
        if (entry) {
          const seeded = ProviderService.buildAccountsFromOpenClawEntries(
            ...
            new Set(),
            defaultModel,
            { [key]: sourceKeys[key] ?? key },
          );
          if (seeded[0]) {
            const normalized = {
              ...kept,
              label: seeded[0].label,
              baseUrl: seeded[0].baseUrl,
              apiProtocol: seeded[0].apiProtocol,
              headers: seeded[0].headers,
              model: seeded[0].model,
              updatedAt: new Date().toISOString(),
            };
            await saveProviderAccount(normalized);
            kept = normalized;
          }
        }
  ...
  static buildAccountsFromOpenClawEntries(
    ...
    for (const [key, entry] of Object.entries(providers)) {
      ...
      const baseUrl = normalizeOpenClawTextValue(entry.baseUrl)
        || normalizeOpenClawTextValue(entry.baseURL)
      ...
      if (defaultModelProvider === key && defaultModel) {
        model = extractOpenClawModelId(defaultModel);
      } else if (Array.isArray(entry.models)) {
        model = extractOpenClawModelId(entry.models[0]);
      } else if (definition?.defaultModelId) {
        model = definition.defaultModelId;
      }

      const sourceKey = sourceKeys?.[key] || key;
      const account: ProviderAccount = {
        ...
        baseUrl,
        apiProtocol: normalizeOpenClawTextValue(entry.api) || definition?.providerConfig?.api,
  ...
  async listLegacyProvidersWithKeyInfo(): Promise<ProviderWithKeyInfo[]> {
    ...
    const authProfileProviders = new Set(await listOpenClawAuthProfileProviders());
    ...
    for (const provider of providers) {
      ...
      const inlineApiKey = normalizeOpenClawTextValue(openClawProviders[runtimeProviderKey]?.apiKey);
      results.push({
        ...provider,
        hasKey: !!apiKey || !!inlineApiKey || authProfileProviders.has(runtimeProviderKey),
