...
import { prepareGatewayLaunchContextWithOverrides } from './config-sync';
...
export class GatewayManager extends EventEmitter {
  ...
  private formatStartupErrorMessage(error: unknown): string {
    const raw = error instanceof Error ? error.message : String(error);
    if (/EACCES|permission denied/i.test(raw)) {
      return `${raw}\nHint: detected executable permission issue. ClawX has retried with Linux/packaged permission self-heal and safe mode. If this persists, move app files to an executable filesystem (not noexec) or reinstall via installer script/package.`;
    }
    if (/Port \d+ still occupied/i.test(raw)) {
      return `${raw}\nHint: another process may still hold the gateway port. ClawX will keep reconnecting automatically; restarting the app can speed up recovery.`;
    }
    if (/connect\.challenge|handshake timeout|ECONNREFUSED/i.test(raw)) {
      return `${raw}\nHint: transient gateway bootstrap issue detected. ClawX has applied retry logic and safe-mode fallback.`;
    }
    return raw;
  }

  private async prepareForSafeModeRetry(): Promise<void> {
    if (this.ws) {
      try {
        this.ws.terminate();
      } catch {
    ...
    clearPendingGatewayRequests(this.pendingRequests, new Error('Gateway switching to safe mode'));
    ...
    this.setStatus({ state: 'starting', error: undefined, pid: undefined });
  }

  private async syncPortFromSettings(): Promise<void> {
    try {
      const configuredPort = Number(await getSetting('gatewayPort'));
      if (!Number.isFinite(configuredPort) || configuredPort <= 0) {
        return;
      }
      const nextPort = Math.trunc(configuredPort);
      if (nextPort !== this.status.port) {
        this.setStatus({ port: nextPort });
      }
    } catch (error) {
      logger.warn('Failed to sync gateway port from settings:', error);
    }
  }
  ...
  async start(): Promise<void> {
    ...
    await this.syncPortFromSettings();
    ...
    try {
      await runGatewayStartupSequence({
        ...
        isSafeModeEnabled: () => this.startupSafeMode,
        enableSafeMode: () => {
          this.startupSafeMode = true;
        },
        prepareForSafeModeRetry: async () => {
          await this.prepareForSafeModeRetry();
        },
        delay: async (ms) => {
      ...
      this.setStatus({ state: 'error', error: this.formatStartupErrorMessage(error) });
      throw error;
    } finally {
  ...
  private async startProcess(): Promise<void> {
    const launchContext = await prepareGatewayLaunchContextWithOverrides(this.status.port, {
      forceServerLite: this.startupSafeMode,
      forceSkipChannels: this.startupSafeMode,
    });
