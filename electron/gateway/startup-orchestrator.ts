...
type StartupHooks = {
  ...
  enableSafeMode: () => void;
  prepareForSafeModeRetry: () => Promise<void>;
  ...
  while (true) {
    ...
    try {
      ...
      const recoveryAction = getGatewayStartupRecoveryAction({
        ...
        safeModeAttempted: hooks.isSafeModeEnabled(),
      ...
      if (recoveryAction === 'safe-mode') {
        hooks.enableSafeMode();
        await hooks.prepareForSafeModeRetry();
        logger.warn(
          'Gateway startup failed with plugin/channel/permission-like error; retrying in safe mode (server-lite + skip channels)',
        );
        await hooks.delay(500);
        continue;
      }
