...
const SAFE_MODE_RECOVERY_PATTERNS: RegExp[] = [
  /failed to load plugin/i,
  /plugin .* failed/i,
  /channel .* failed/i,
  /extension .* failed/i,
  /invalid token/i,
  /spawn .* (EACCES|EPERM)/i,
  /permission denied/i,
...
export type GatewayStartupRecoveryAction = 'repair' | 'retry' | 'safe-mode' | 'fail';

export function isSafeModeCandidateError(error: unknown): boolean {
  const errorText = error instanceof Error
    ? `${error.name}: ${error.message}`
    : String(error ?? '');
  return SAFE_MODE_RECOVERY_PATTERNS.some((pattern) => pattern.test(errorText));
}
...
export function getGatewayStartupRecoveryAction(options: {
  ...
  if (!options.safeModeAttempted) {
    const errorText = options.startupError instanceof Error
      ? `${options.startupError.name}: ${options.startupError.message}`
      : String(options.startupError ?? '');
    const stderrText = options.startupStderrLines.join('\n');
    const combined = `${errorText}\n${stderrText}`;
    if (SAFE_MODE_RECOVERY_PATTERNS.some((pattern) => pattern.test(combined))) {
      return 'safe-mode';
    }
  }
