...
export function Channels() {
  ...
  return (
    <div data-testid="channels-page" className="flex flex-col h-full overflow-hidden bg-background">
            ...
            <div className="mb-12">
              ...
              <div className="space-y-4">
                {configuredGroups.map((group) => (
                  <div key={group.channelType} className="rounded-2xl border border-border/70 p-4 bg-card shadow-sm">
                      ...
                      {group.accounts.map((account) => {
                        const displayName =
                          account.accountId === 'default' && account.name === account.accountId
                            ...
                            : account.name;
                        return (
                        <div key={`${group.channelType}-${account.accountId}`} className="rounded-xl bg-background border border-border/40 px-4 py-3 shadow-sm">
              ...
              {unsupportedGroups.map((type) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'group flex items-start gap-4 p-4 rounded-2xl transition-all text-left border relative overflow-hidden border-border/45 bg-card hover:bg-card'
                    )}
                  >
                    <div className="h-[46px] w-[46px] shrink-0 flex items-center justify-center text-foreground bg-background border border-border/60 rounded-full shadow-sm mb-3">
                    ...
                    <div className="flex flex-col flex-1 min-w-0 py-0.5 mt-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {meta.isPlugin && (
                          <Badge variant="secondary" className="font-mono text-[10px] font-medium px-2 py-0.5 rounded-full bg-background border border-border/50 shadow-none text-foreground/70">
                            {t('pluginBadge')}
                          </Badge>
                        )}
                      </div>
