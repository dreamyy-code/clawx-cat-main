...
function normalizeVisibleAssistantText(message: RawMessage): string {
  return extractText(message).replace(/\s+/g, ' ').trim();
}

function getVisibleMessageRichness(message: RawMessage): number {
  const text = normalizeVisibleAssistantText(message);
  const thinking = (extractThinking(message) || '').replace(/\s+/g, ' ').trim();
  const tools = extractToolUse(message).length;
  const images = extractImages(message).length;
  const attachments = Array.isArray(message._attachedFiles) ? message._attachedFiles.length : 0;
  return text.length + thinking.length + tools * 10 + images * 10 + attachments * 10;
}

function dedupeVisibleAssistantMessages(messages: RawMessage[]): RawMessage[] {
  ...
  for (const message of messages) {
    ...
    const lastText = normalizeVisibleAssistantText(last);
    const currentText = normalizeVisibleAssistantText(message);
    if (!lastText || !currentText || lastText !== currentText) {
      visible.push(message);
      continue;
    }
    if (getVisibleMessageRichness(message) >= getVisibleMessageRichness(last)) {
...
export function Chat() {
  ...
  const shouldHideTechnicalError = Boolean(
    error
    && (
      error.includes('RPC timeout: chat.history')
  ...
  const visibleMessages = dedupeVisibleAssistantMessages(messages);

  const isEmpty = visibleMessages.length === 0 && !sending;
  const subagentCompletionInfos = visibleMessages.map((message) => parseSubagentCompletionInfo(message));
  const nextUserMessageIndexes = new Array<number>(visibleMessages.length).fill(-1);
  ...
  for (let idx = visibleMessages.length - 1; idx >= 0; idx -= 1) {
    ...
    if (visibleMessages[idx].role === 'user' && !subagentCompletionInfos[idx]) {
      nextUserMessageIndex = idx;
    }
  }

  const userRunCards = visibleMessages.flatMap((message, idx) => {
    if (message.role !== 'user' || subagentCompletionInfos[idx]) return [];

    const nextUserIndex = nextUserMessageIndexes[idx];
    const segmentEnd = nextUserIndex === -1 ? visibleMessages.length : nextUserIndex;
    const segmentMessages = visibleMessages.slice(idx + 1, segmentEnd);
    ...
    return [{
      ...
      segmentEnd: nextUserIndex === -1 ? visibleMessages.length - 1 : nextUserIndex - 1,
      steps,
    }];
  });
  ...
  return (
    <div className={cn("relative flex min-h-0 flex-col -m-6 transition-colors duration-500 dark:bg-background")} style={{ height: 'calc(100vh - 2.5rem)' }}>
      ...
      {/* Error bar */}
      {error && !shouldHideTechnicalError && (
      ...
      {minLoading && !sending && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-background  rounded-xl pointer-events-auto">
    ...
    <div className="flex flex-col items-center justify-center text-center h-[60vh]">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2.5 max-w-lg w-full">
        {quickActions.map(({ key, label }) => (
          <button 
            key={key}
            className="px-4 py-1.5 rounded-full border border-border dark:border-white/10 text-[13px] font-medium text-foreground/70 hover:bg-black/5 dark:hover:bg-white/5 transition-colors bg-black/[0.02]"
          >
            {label}
          </button>
        ))}
