...
export function ChatInput({ onSend, onStop, disabled = false, sending = false, isEmpty = false }: ChatInputProps) {
  ...
  const presetTargetAgentId = useChatStore((s) => s.presetTargetAgentId);
  ...
  useEffect(() => {
    if (!presetTargetAgentId) {
      return;
    }
    if (presetTargetAgentId === 'main' || presetTargetAgentId === currentAgentId) {
      setTargetAgentId(null);
      setPresetTargetAgentId(null);
      return;
    }
    if (!(agents ?? []).some((agent) => agent.id === presetTargetAgentId)) {
      setPresetTargetAgentId(null);
      return;
    }
    setTargetAgentId(presetTargetAgentId);
    setPickerOpen(false);
    setPresetTargetAgentId(null);
    textareaRef.current?.focus();
  }, [agents, currentAgentId, presetTargetAgentId, setPresetTargetAgentId]);
  ...
  const handleSend = useCallback(() => {
    ...
    setPresetTargetAgentId(null);
    setPickerOpen(false);
  }, [input, attachments, canSend, onSend, setPresetTargetAgentId, targetAgentId]);
  ...
  const handleKeyDown = useCallback(
    ...
    [handleSend, input, setPresetTargetAgentId, targetAgentId],
  );
  ...
  return (
    ...
    >
      <div className="w-full">
        ...
        {/* Input Row */}
        <div className={`relative bg-card rounded-[28px] shadow-sm border p-1.5 transition-all  ${dragOver ? 'border-primary ring-1 ring-primary' : 'border-border/70'}`}>
          {selectedTarget && (
          ...
          <div className="flex items-end gap-1.5">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="shrink-0 h-10 w-10 rounded-full text-muted-foreground hover:bg-card hover:text-foreground transition-colors"
              onClick={pickFiles}
            ...
            {showAgentPicker && (
              <div ref={pickerRef} className="relative shrink-0">
                <Button
                  ...
                  size="icon"
                  className={cn(
                    'h-10 w-10 rounded-full text-muted-foreground hover:bg-card hover:text-foreground transition-colors',
                ...
                {pickerOpen && (
                  <div className="absolute left-0 bottom-full z-20 mb-2 w-72 overflow-hidden rounded-2xl border border-border/70 bg-card p-1.5 shadow-xl ">
            ...
            <Button
              ...
              className={`shrink-0 h-10 w-10 rounded-full transition-colors ${
                (sending || canSend)
                  ...
                  : 'text-muted-foreground/50 hover:bg-background bg-transparent'
              }`}
              variant="ghost"
...
function AttachmentPreview({
  ...
  return (
    <div className="relative group rounded-xl overflow-hidden border border-border/40 shadow-sm">
      {isImage ? (
        // Image thumbnail
