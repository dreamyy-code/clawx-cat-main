            ...
            {attachedFiles.map((file, i) => {
              ...
              if (isImage) {
                return file.preview ? (
                  ...
                  <div
                    key={`local-${i}`}
                    className="w-36 h-36 rounded-2xl border border-border/40 bg-black/5 dark:bg-white/5 flex items-center justify-center text-muted-foreground shadow-sm"
                  >
        ...
        {!isUser && attachedFiles.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {attachedFiles.map((file, i) => {
              ...
              if (isImage && !file.preview) {
                return (
                  <div key={`local-${i}`} className="w-36 h-36 rounded-xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 flex items-center justify-center text-muted-foreground">
                    <File className="h-8 w-8" />
                  </div>
                );
              }
        ...
        return (
          <div
            ...
            className={cn(
              ...
              !isRunning && !isError && 'border-border/40 bg-muted/20 text-muted-foreground',
          ...
          <ReactMarkdown
            ...
            components={{
              code({ className, children, ...props }) {
                ...
                if (isInline) {
                  return (
                    <code className="bg-background px-1.5 py-0.5 rounded text-sm font-mono break-words break-all" {...props}>
                      {children}
                    </code>
                  );
                }
                return (
                  <pre className="bg-background rounded-2xl border border-border/40 shadow-inner p-4 overflow-x-auto">
...
function ThinkingBlock({ content }: { content: string }) {
  ...
  return (
    <div className="w-full rounded-2xl border border-border/40 bg-card text-[14px] shadow-sm">
      <button
...
function FileCard({ file }: { file: AttachedFileMeta }) {
  ...
  return (
    <div 
      className={cn(
        "flex items-center gap-3 rounded-xl border border-border/70 px-3 py-2.5 bg-card max-w-[220px] shadow-sm",
        file.filePath && "cursor-pointer hover:bg-card transition-colors"
      )}
      onClick={handleOpen}
...
function ImageThumbnail({
  ...
  return (
    <div
      className="relative w-36 h-36 rounded-2xl border overflow-hidden border-border/40 bg-black/5 dark:bg-white/5 group/img cursor-zoom-in shadow-sm"
      onClick={onPreview}
    >
...
function ImagePreviewCard({
  ...
  return (
    <div
      className="relative max-w-xs rounded-2xl border overflow-hidden border-border/40 bg-black/5 dark:bg-white/5 group/img cursor-zoom-in shadow-sm"
      onClick={onPreview}
    >
...
function ToolCard({ name, input }: { name: string; input: unknown }) {
  ...
  return (
    <div className="rounded-xl border border-border dark:border-white/10 bg-black/5 dark:bg-white/5 text-[14px]">
      <button
