...
function StepDetailCard({ step }: { step: TaskStep }) {
  ...
  return (
    <div className="min-w-0 flex-1 rounded-xl border border-border bg-white/40 px-3 py-2 dark:border-white/10 dark:bg-white/[0.03]">
      <button
        type="button"
      ...
      {step.detail && expanded && (
        <div className="mt-3 rounded-xl border border-border/40 bg-black/[0.03] px-3 py-2 dark:border-white/10 dark:bg-white/[0.03] shadow-inner">
...
export function ExecutionGraphCard({
  ...
  return (
    <div
      ...
      className="w-full rounded-2xl border border-border bg-card px-4 py-4 shadow-sm dark:border-white/10 dark:bg-white/[0.04]"
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => setCollapsed((value) => !value)}
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border/60 bg-white/70 text-muted-foreground transition-colors hover:text-foreground dark:border-white/10 dark:bg-white/[0.05]"
            aria-label={collapsed ? 'Expand execution graph' : 'Collapse execution graph'}
            title={collapsed ? 'Expand execution graph' : 'Collapse execution graph'}
          >
