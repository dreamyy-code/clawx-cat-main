...
function attachedFilesCount(message: RawMessage): number {
  return Array.isArray(message._attachedFiles) ? message._attachedFiles.length : 0;
}

function isSemanticDuplicateAssistantMessage(
  existing: RawMessage,
  incoming: RawMessage,
  timeWindowMs = 10_000,
): boolean {
  if (existing.role !== (incoming.role || 'assistant')) return false;
  const existingText = normalizeComparableMessageText(existing.content);
  const incomingText = normalizeComparableMessageText(incoming.content);
  if (!existingText || !incomingText || existingText !== incomingText) return false;
  // If incoming message carries richer attachment payload, keep it.
  if (attachedFilesCount(incoming) > attachedFilesCount(existing)) return false;
  if (typeof existing.timestamp !== 'number' || typeof incoming.timestamp !== 'number') return true;
  return Math.abs(toMs(existing.timestamp) - toMs(incoming.timestamp)) <= timeWindowMs;
}

function dedupeSemanticAssistantMessages(messages: RawMessage[]): RawMessage[] {
  ...
  for (const message of messages) {
    ...
    const index = deduped.findIndex((existing) => isSemanticDuplicateAssistantMessage(existing, message));
    if (index === -1) {
      deduped.push(message);
      continue;
    }
    const existing = deduped[index];
    const preferIncoming = attachedFilesCount(message) > attachedFilesCount(existing);
    if (preferIncoming) {
...
const CHAT_SEND_SAFETY_TIMEOUT_MS = 180_000;
const CHAT_SEND_RECOVERY_RETRY_MS = 15_000;
const CHAT_SEND_MAX_STUCK_RECOVERY_ATTEMPTS = 2;

function isIgnorableChatHistoryTimeout(error: string | null): boolean {
  if (!error) return false;
  return error.includes('RPC timeout: chat.history')
    || error.includes('Gateway RPC timeout: chat.history');
}
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  setPresetTargetAgentId: (agentId: string | null) => {
    set({ presetTargetAgentId: agentId });
  },
  ...
  newSession: (baseAgentId?: string) => {
    ...
    const requestedAgentId = normalizeAgentId(baseAgentId);
    const requestedPrefix = getCanonicalPrefixFromSessionKey(
      resolveMainSessionKeyForAgent(requestedAgentId) ?? DEFAULT_SESSION_KEY,
    );
  ...
  loadHistory: async (quiet = false) => {
    ...
    const loadPromise = (async () => {
      ...
      const applyLoadFailure = (errorMessage: string | null) => {
        ...
        const shouldHideError = isIgnorableChatHistoryTimeout(errorMessage);
        set((state) => {
          const hasMessages = state.messages.length > 0;
          return {
            loading: false,
            error: !quiet && errorMessage && !shouldHideError ? errorMessage : (shouldHideError ? null : state.error),
      ...
      const dedupedMessages = dedupeSemanticAssistantMessages(enrichedMessages);

      // Preserve the optimistic user message during an active send.
      // The Gateway may not include the user's message in chat.history
      // until the run completes, causing it to flash out of the UI.
      let finalMessages = dedupedMessages;
      const userMsgAt = get().lastUserMessageAt;
      if (get().sending && userMsgAt) {
        const userMsMs = toMs(userMsgAt);
        const hasRecentUser = dedupedMessages.some(
        ...
        if (!hasRecentUser) {
          ...
          if (optimistic) {
            finalMessages = [...dedupedMessages, optimistic];
          }
        }
      }
  ...
  sendMessage: async (
    ...
    const finalizeStuckSend = (error: string) => {
      clearHistoryPoll();
      set({
        error,
        sending: false,
    ...
    const recoverAfterChatSendTimeout = (errorSource: string) => {
      console.warn(`[sendMessage] Recoverable chat.send timeout from ${errorSource}; continuing to poll for completion`);
      set({ error: null });
      void get().loadHistory(true);
      void useGatewayStore.getState().checkHealth().then((health) => {
        if (!get().sending) return;
        if (!health.ok) {
          console.warn(
            `[sendMessage] Gateway health probe after ${errorSource} timeout reported unhealthy: ${health.error || 'unknown error'}`,
          );
        }
      }).catch((healthError) => {
        console.warn('[sendMessage] Gateway health probe after recoverable timeout failed:', healthError);
      });
    };
    const checkStuck = () => {
      const state = get();
      ...
      if (Date.now() - _lastChatEventAt < CHAT_SEND_SAFETY_TIMEOUT_MS) {
        setTimeout(checkStuck, 10_000);
        return;
      }
      ...
      void Promise.allSettled([
        state.loadHistory(true),
        useGatewayStore.getState().checkHealth(),
      ]).then((results) => {
        if (!get().sending) return;
        if (Date.now() - _lastChatEventAt < CHAT_SEND_SAFETY_TIMEOUT_MS) {
          setTimeout(checkStuck, 10_000);
        ...
        if (stuckRecoveryAttempts <= CHAT_SEND_MAX_STUCK_RECOVERY_ATTEMPTS) {
          if (!health?.ok) {
            console.warn(
              `[sendMessage] Stuck-send recovery probe ${stuckRecoveryAttempts}/${CHAT_SEND_MAX_STUCK_RECOVERY_ATTEMPTS} reported unhealthy gateway: ${health?.error || 'unknown error'}`,
            );
          }
          setTimeout(checkStuck, CHAT_SEND_RECOVERY_RETRY_MS);
          return;
        }

        const gatewayState = useGatewayStore.getState().status.state;
        const healthError = health && !health.ok ? health.error : null;
        finalizeStuckSend(
          healthError
            ? `Gateway timeout while waiting for OpenClaw: ${healthError}`
            : gatewayState !== 'running'
              ? 'Gateway is no longer running. Please restart Gateway and try again.'
              : 'OpenClaw is still not responding. Please retry, or restart Gateway if this keeps happening.',
        );
      }).catch((recoveryError) => {
        console.warn('[sendMessage] Stuck-send recovery probe failed:', recoveryError);
        if (stuckRecoveryAttempts <= CHAT_SEND_MAX_STUCK_RECOVERY_ATTEMPTS) {
          setTimeout(checkStuck, CHAT_SEND_RECOVERY_RETRY_MS);
          return;
        }
        finalizeStuckSend(
          'OpenClaw did not respond in time. Please retry, or restart Gateway if this keeps happening.',
        );
      });
    };
    ...
    try {
      ...
      if (!result.success) {
        ...
        if (isRecoverableChatSendTimeout(errorMsg)) {
          recoverAfterChatSendTimeout('rpc-result');
        } else {
          clearHistoryPoll();
      ...
      if (isRecoverableChatSendTimeout(errStr)) {
        recoverAfterChatSendTimeout('rpc-exception');
      } else {
        clearHistoryPoll();
      ...
      case 'final': {
        ...
        if (finalMsg) {
            ...
            set((s) => {
              ...
              if (currentStream) {
                ...
                if (streamRole === 'assistant' || streamRole === undefined) {
                  ...
                  const streamAsAssistant: RawMessage = {
                    ...(currentStream as RawMessage),
                    role: 'assistant',
                    id: snapId,
                  };
                  const snapshotExists = s.messages.some((m) =>
                    m.id === snapId || isSemanticDuplicateAssistantMessage(m, streamAsAssistant),
                  );
                  if (!snapshotExists) {
                    snapshotMsgs.push({
                      ...streamAsAssistant,
          ...
          set((s) => {
            ...
            // Some runtimes can emit the same final answer with different IDs.
            const alreadyExists = s.messages.some((m) =>
              m.id === msgId || isSemanticDuplicateAssistantMessage(m, msgWithImages),
            );
            if (alreadyExists) {
              return toolOnly ? {
                streamingText: '',
