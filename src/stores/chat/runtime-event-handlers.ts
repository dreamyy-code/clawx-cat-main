        ...
        case 'final': {
          ...
          if (finalMsg) {
            ...
            set((s) => {
              ...
              // Some runtimes can deliver the same final assistant turn via two
              // channels with different/empty IDs, so we also dedupe by content
              // + role + close timestamp.
              const messageText = getMessageText(msgWithImages.content).trim();
              const messageTs = typeof msgWithImages.timestamp === 'number' ? msgWithImages.timestamp : null;
              const alreadyExists = s.messages.some((m) => {
                if (m.id === msgId) return true;
                if (m.role !== msgWithImages.role) return false;
                const existingText = getMessageText(m.content).trim();
                if (!messageText || !existingText || existingText !== messageText) return false;
                if (messageTs == null || typeof m.timestamp !== 'number') return true;
                return Math.abs(toMs(m.timestamp) - toMs(messageTs)) <= 10_000;
              });
              if (alreadyExists) {
                return toolOnly ? {
                  streamingText: '',
