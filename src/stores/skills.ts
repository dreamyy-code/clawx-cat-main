...
interface SkillsState {
  ...
  installSkill: (slug: string, version?: string, options?: { targetMode?: 'global' | 'agent'; agentId?: string }) => Promise<void>;
...
export const useSkillsStore = create<SkillsState>((set, get) => ({
  ...
  installSkill: async (slug: string, version?: string, options?: { targetMode?: 'global' | 'agent'; agentId?: string }) => {
    set((state) => ({ installing: { ...state.installing, [slug]: true } }));
    try {
      const result = await hostApiFetch<{ success: boolean; error?: string }>('/api/clawhub/install', {
        method: 'POST',
        body: JSON.stringify({ slug, version, targetMode: options?.targetMode, agentId: options?.agentId }),
      });
      if (!result.success) {
